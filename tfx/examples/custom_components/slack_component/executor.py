...
class Executor(base_executor.BaseExecutor):
  ...
  def _fetch_slack_blessing(self, slack_token, channel_id,
                            model_uri):
    ...
    msg = 'Please review the model in the following URI: {}'.format(model_uri)
    ts = -1
    if sc.rtm_connect():
      ...
      while sc.server.connected:
        ...
        for payload in payload_list:
          if payload.get('ok') and payload.get('reply_to') == 0:
            ts = payload['ts']
            continue
  ...
  def Do(self, input_dict,
    ...
    timeout = exec_properties['timeout']
